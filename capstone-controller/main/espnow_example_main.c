...
static const char *TAG = "sherlock_controller";
...
int unicast_espnow_data_parse(uint8_t *data, uint16_t data_len, uint8_t *mac_addr)
{
    ...
    if(type == UNICAST_TYPE_ENVIRONMENT) {
        ...
        crc = buf->crc;
        buf->crc = 0;
        crc_cal = esp_crc16_le(UINT16_MAX, (uint8_t const *)buf, sizeof(espnow_environment_data_t));
        if (crc_cal == crc) {
            recent_key_value = buf->key;
...
void broadcast_espnow_data_prepare(espnow_send_param_t *send_param)
{
...
static esp_err_t espnow_init(void)
{
    ...
    xTaskCreate(espnow_task, "controller_task", 2048, send_param, 4, NULL);
